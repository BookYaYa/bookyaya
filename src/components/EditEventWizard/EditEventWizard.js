...
import EditEventWizardTab, { DESCRIPTION, POLICY } from './EditEventWizardTab';
import css from './EditEventWizard.css';

// TODO: PHOTOS panel needs to be the last one since it currently contains PayoutDetailsForm modal
// All the other panels can be reordered.
export const TABS = [DESCRIPTION, POLICY];
...
const tabLabel = (intl, tab) => {
  let key = null;
  if (tab === DESCRIPTION) {
    key = 'EditEventWizard.tabLabelDescription';
  } else if (tab === POLICY) {
    key = 'EditEventWizard.tabLabelPolicy';
  }
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    ...
    case POLICY:
      return true; /** TODO: add validation for policy */
    default:
      return false;
  }
};
...
// Create a new or edit listing through EditEventWizard
class EditEventWizard extends Component {
  constructor(props) {
    super(props);
  ...
  handlePayoutSubmit(values) {
    ...
    this.props
      ...
      .then(() => {
        this.setState({ showPayoutDetails: false });
        this.props.onManageDisableScrolling('EditEventWizard.payoutModal', false);
  ...
  render() {
    ...
    if (!tabsStatus[selectedTab]) {
      return <NamedRedirect name="EditEventPage" params={{ ...params, tab: TABS[0] }} />;
    }
    ...
    const tabLink = tab => {
      return { name: 'EditEventPage', params: { ...params, tab } };
    };
    ...
    return (
      <div className={classes}>
        ...
        <Modal
          id="EditEventWizard.payoutModal"
...
EditEventWizard.defaultProps = {
  className: null,
  rootClassName: null,
  listing: null,
};

EditEventWizard.propTypes = {
...
export default compose(withViewport, injectIntl)(EditEventWizard);
